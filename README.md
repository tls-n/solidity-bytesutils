...
# Bytes utility library for Solidity
## Notes
* Based on https://github.com/Arachnid/solidity-stringutils
* When talking about strings, we mean bytestrings.
* Major differences are `toBytes()` and `truncate()`
...
### Getting the character length of a string
    var len = "Test Bytes 123".toSlice().len(); // 14
...
### Splitting a string into an array
    ...
    var delim = ".".toSlice();
    var parts = new bytes[](s.count(delim) + 1);
    for(uint i = 0; i < parts.length; i++) {
        parts[i] = s.split(delim).toBytes();
    }

### Extracting the middle part of a string
    var s = "www.google.com".toSlice();
    bytesutils.slice memory part;
...
### Converting a slice back to a bytestring
    var myString = mySlice.toBytes();
...
### toSlice(bytes self) internal returns (slice)
...
### toBytes(slice self) internal returns (bytes)
...
Returns Newly allocated bytes containing the slice's text.
...
### concat(slice self, slice other) internal returns (bytes)
...
### join(slice self, slice[] parts) internal returns (bytes)
...
### truncate(slice self, uint new_len) internal returns (slice)
    
Truncates a slice to a specified new length.

Arguments:

 - self The slice to truncate.
 - parts The new length.

Returns the truncated slice.
